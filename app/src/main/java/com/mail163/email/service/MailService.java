...
public class MailService extends Service {
    ...
    private static final String EXTRA_CLR_EMAIL = "com.mail163.email.intent.extra.CLREMAIL";
    ...
    public StoreAndStreamTask mStoreAndStreamTask;
    public StoreSizeCheckTask mStoreSizeCheckTask;
    private int mStartId;

    public StoreMessageHandler messageHandler = new StoreMessageHandler();
    /**
    ...
    public static void actionClearAccount(Context context) {
        Intent i = new Intent();
        i.setClass(context, MailService.class);
        i.setAction(MailService.EXTRA_CLR_EMAIL);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (EXTRA_CLR_EMAIL.equals(action)) {
            Utiles.clearAccount(this);
        }
            ...
            Logs.d(LOG_TAG, "action: cancel");
            cancel();
            stopSelf(startId);
        }
        else if (ACTION_RESCHEDULE.equals(action)) {
            Logs.d(LOG_TAG, "action: reschedule");
            ...
            Logs.d(LOG_TAG, "notify accountId=" + Long.toString(accountId)
    ...
    public void onDestroy() {
        ...
        Utility.cancelTaskInterrupt(mStoreAndStreamTask);
        mStoreAndStreamTask = null;
        Utility.cancelTaskInterrupt(mStoreSizeCheckTask);
        mStoreSizeCheckTask = null;
    }
    ...
    private boolean syncOneAccount(Controller controller, long checkAccountId, int startId) {
        Logs.d(Logs.LOG_TAG, "syncOneAccount");
        long inboxId = Mailbox.findMailboxOfType(this, checkAccountId, Mailbox.TYPE_INBOX);
        Logs.d(Logs.LOG_TAG, "syncOneAccount inboxId :"+inboxId);
    ...
    /* package */ AccountSyncReport updateAccountReport(long accountId, int newCount) {
        ...
        synchronized (mSyncReports) {
            ...
            if (Config.LOGD && Email.DEBUG) {
                Logs.d(LOG_TAG, "update account " + report.toString());
            }
            return report;
        }
    }
    ...
    /* package */ void restoreSyncReports(Intent restoreIntent) {
        ...
        synchronized (mSyncReports) {
            ...
            if (accountInfo == null) {
//                Logs.d(LOG_TAG, "no data in intent to restore");
                return;
            }
            int accountInfoIndex = 0;
    ...
    private class StoreAndStreamTask extends AsyncTask<String, Void, Integer> {
        private static final int STATE_FINISH = 1;
        private static final int Stream_TASK = 2;
        @Override
        protected void onPreExecute() {
        }
        @Override
        protected Integer doInBackground(String... params) {
            long currentStream = Utiles.getNetStreamSize(MailService.this);

            if(currentStream > changeSize(Global.set_streamSize)){
                return Stream_TASK;
            }
            return STATE_FINISH;
        }
        ...
        protected void onPostExecute(Integer result) {
            ...
            switch (state) {
                case Stream_TASK:
                    Utiles.showNoNetworkNotification(MailService.this,
                            getString(R.string.stream_alarm_title),String.format(getString(R.string.stream_alarm),Global.set_streamSize+"0M"));
                    break;
                case STATE_FINISH:
                    break;
                default:
    ...
    private class StoreSizeCheckTask extends AsyncTask<String, Void, Integer> {
        private static final int STATE_FINISH = 1;
        private static final int MOBILE_TASK = 3;
        private static final int SDCARD_TASK = 4;
        @Override
        protected void onPreExecute() {
        }
        @Override
        protected Integer doInBackground(String... params) {
            if(Global.set_store == Global.set_mobile){
                long currentAvailMemory = Utiles.getAvailMemorySize(MailService.this);
                if(currentAvailMemory < changeSize(Global.set_sdSize)){
                    return MOBILE_TASK;
                }
            }else if(Global.set_store == Global.set_sdcard){
                long currentSDcardSize = Utiles.getSdcardAvailSizes(MailService.this);
                if(currentSDcardSize < changeSize(Global.set_sdSize)){
                    return SDCARD_TASK;
                }
            }
            return STATE_FINISH;
        ...
        protected void onPostExecute(Integer result) {
            ...
            switch (state) {
                case MOBILE_TASK:
                    Utiles.showNoNetworkNotification(MailService.this,
                            getString(R.string.store_alarm_title),String.format(getString(R.string.store_alarm),"手机数据",Global.set_sdSize+"0M"));
                    break;
                case SDCARD_TASK:
                    Utiles.showNoNetworkNotification(MailService.this,
                            getString(R.string.store_alarm_title),String.format(getString(R.string.store_alarm),"SD卡",Global.set_sdSize+"0M"));
                    break;
                case STATE_FINISH:
                    break;
                default:
    ...
    private long changeSize(int k){
        switch(k){
            case Global.set_streamSize_0 :
                return 0;
            case Global.set_streamSize_10 :
                return 10 * 1024;
            case Global.set_streamsize_20 :
                return 20 * 1024;
            case Global.set_streamsize_30 :
                return 30 * 1024;
        }
        return 30 * 1024;
    }
    private class StoreMessageHandler extends Handler {
        private static final int MSG_STORE_PROGRESS = 1;
        @Override
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case MSG_STORE_PROGRESS:
                    if(Global.set_streamSize > 0){ //== 0 关闭检查
                        mStoreAndStreamTask = (StoreAndStreamTask)new StoreAndStreamTask().execute("");
                    }
                    if(Global.set_sdSize > 0){//== 0 关闭检查
                        mStoreSizeCheckTask = (StoreSizeCheckTask)new StoreSizeCheckTask().execute("");
                    }
                    break;
    ...
    private void notifyNewMessages(long accountId) {
        ...
        builder.setContentTitle(getString(R.string.notification_new_title));
        builder.setContentText(reportString);   //通知内容
        builder.setTicker(getString(R.string.notification_new_title));    //刚收到通知时提示
        builder.setNumber(numNewMessages);                 //消息个数
        builder.setAutoCancel(true);          // 点击取消通知

        builder.setWhen(System.currentTimeMillis());
        builder.setSound(ringtone);               //通知声音
        builder.setContentIntent(pending);  //通知点击行为
...
//        notification.number = numNewMessages;
...
//            notification.defaults |= Notification.DEFAULT_VIBRATE;
            builder.setDefaults(Notification.DEFAULT_VIBRATE);// requires VIBRATE permission
        }
...
//        notification.defaults |= Notification.DEFAULT_LIGHTS;
        ...
        NotificationManager notificationManager =
                (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        notificationManager.notify(NOTIFICATION_ID_NEW_MESSAGES, builder.build());
    }
}
