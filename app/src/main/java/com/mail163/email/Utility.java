...
//        }
    ...
    public static String buildSearch(ContentResolver resolver, String searchCondition, String mailboxIdBuffer) {
        StringBuilder selection = new StringBuilder("("
                + MessageColumns.DISPLAY_NAME + " LIKE '%" + searchCondition + "%'"
                +" or "
                + MessageColumns.SUBJECT + " LIKE '%" + searchCondition + "%'"
                +" or ");
        StringBuilder inboxes = new StringBuilder();
        Cursor c = resolver.query(EmailContent.Body.CONTENT_URI,
                new String[]{EmailContent.Body.MESSAGE_KEY},
                buildSearchBody(searchCondition),
                null, null);
        // build an IN (mailboxId, ...) list
        // TODO do this directly in the provider
        while (c.moveToNext()) {
        ...
        c.close();
        selection.append(EmailContent.RECORD_ID + " IN ");
        selection.append("(").append(inboxes).append(")");

        selection.append(") and ");
        selection.append(MessageColumns.MAILBOX_KEY + " IN ");
        selection.append("(").append(mailboxIdBuffer).append(")");

        return selection.toString();
    }

    public static String buildSearchBody(String searchCondition) {
        StringBuilder selection = new StringBuilder(
                BodyColumns.HTML_CONTENT + " LIKE '%" + searchCondition + "%'"
                        +" or "
                        + BodyColumns.TEXT_CONTENT + " LIKE '%" + searchCondition + "%'");
