...
import com.mail163.email.provider.EmailContent.HostAuthColumns;
...
public class EmailProvider extends ContentProvider {
    ...
    // Version 13: Add priority to message table. content is deprecated.
    // Version 14: Add EmailContact table.
    ...
    public static final String EMAIL_AUTHORITY = "com.mail163.email.provider";
    ...
    //Email contact
    private static final int EMAIL_CONTACT_BASE = 0x8000;
    private static final int EMAIL_CONTACT = EMAIL_CONTACT_BASE;
    private static final int EMAIL_CONTACT_ID = EMAIL_CONTACT_BASE + 1;
    ...
    static {
        ...
        // Email Contact
        matcher.addURI(EMAIL_AUTHORITY, "emailcontact", EMAIL_CONTACT);
        // A specific updated message
        matcher.addURI(EMAIL_AUTHORITY, "emailcontact/#", EMAIL_CONTACT_ID);
    }
    ...
    static void createMessageTable(SQLiteDatabase db) {
        String messageColumns = MessageColumns.DISPLAY_NAME + " text, "
            ...
            + MessageColumns.PRIORITY + " integer"
            + ");";
    ...
    static void createEmailContactTable(SQLiteDatabase db) {
        Log.d(TAG, "Creating createEmailContactTable database");
        String s = " (" + EmailContent.RECORD_ID + " integer primary key autoincrement, "
            + EmailContactColumns.USERNAME + " text, "
            + EmailContactColumns.EMAIL + " text"
            + ");";
        db.execSQL("create table " + EmailContact.TABLE_NAME + s);
    }

    static void resetEmailContactTable(SQLiteDatabase db, int oldVersion, int newVersion) {
        try {
            db.execSQL("drop table " + EmailContact.TABLE_NAME);
        } catch (SQLException e) {
        }
        createEmailContactTable(db);
    }
    ...
    private class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion < 5) {
                ...
                resetEmailContactTable(db, oldVersion, newVersion);
                return;
            }
            if (oldVersion == 5) {
            ...
            Logs.d(TAG, "oldVersion :"+oldVersion);
            if (oldVersion == 12) {
                // Message: add PRIORITY
                try {
                	 db.execSQL("alter table " + Message.UPDATED_TABLE_NAME
                             + " add column " + MessageColumns.PRIORITY + " integer" + ";");
        ...
        try {
            switch (match) {
                ...
                case EMAIL_CONTACT:
                    ...
                    break;
                case HOSTAUTH:
                    id = db.insert(TABLE_NAMES[table], "contact", values);
                    resultUri = ContentUris.withAppendedId(uri, id);
