...
import com.mail163.email.mail.FetchProfile;
import com.mail163.email.mail.Flag;
import com.mail163.email.mail.Folder;
import com.mail163.email.mail.Message;
import com.mail163.email.mail.MessagingException;
import com.mail163.email.mail.Part;
import com.mail163.email.mail.Sender;
import com.mail163.email.mail.Store;
import com.mail163.email.mail.StoreSynchronizer;
import com.mail163.email.mail.Folder.FolderType;
import com.mail163.email.mail.Folder.MessageRetrievalListener;
import com.mail163.email.mail.Folder.OpenMode;
import com.mail163.email.mail.internet.MimeBodyPart;
import com.mail163.email.mail.internet.MimeHeader;
import com.mail163.email.mail.internet.MimeMultipart;
...
import com.mail163.email.provider.EmailContent.AttachmentColumns;
...
import com.mail163.email.provider.EmailContent.SyncColumns;
...
public class MessagingController implements Runnable {
        ...
        put("listFolders", listener, new Runnable() {
            public void run() {
                try {
                    ...
                    int reCount = EmailContent.count(mContext,  
                    		Mailbox.CONTENT_URI, MessageColumns.ACCOUNT_KEY + "=?", new String[] { String.valueOf(accountId)});
           
                    if(reCount > 0){
                    	return;
                    }
                    ArrayList<String> localHashSet = new ArrayList<String>();
                    
                    localHashSet.add(mContext.getString(R.string.mailbox_name_server_nexbox));
                    localHashSet.add(mContext.getString(R.string.mailbox_name_server_inbox));
                    localHashSet.add(mContext.getString(R.string.mailbox_name_server_drafts));
                    localHashSet.add(mContext.getString(R.string.mailbox_name_server_outbox));
                    localHashSet.add(mContext.getString(R.string.mailbox_name_server_sent));
                    localHashSet.add(mContext.getString(R.string.mailbox_name_server_trash));
                   
                    
                    for (String remoteNameToAdd : localHashSet) {
    ...
    private void synchronizeMailboxSynchronous(final EmailContent.Account account,
        ...
        try {
            ...
            Logs.d(Logs.LOG_TAG, "synchronizeMailboxSynchronous customSync : "+customSync);
            if (customSync == null) {
    ...
    private StoreSynchronizer.SyncResults synchronizeMailboxGeneric(
        ...
        Logs.d(Logs.LOG_TAG, "*** synchronizeMailboxGeneric ***");
        Utiles.fetchDownMailSetting(mContext);
        ...
        try {
            localUidCursor = resolver.query(
                    ...
                    null);
            Logs.d(Logs.LOG_TAG, "localUidCursor count :"+localUidCursor.getCount());
        ...
        Logs.d(Logs.LOG_TAG, "remoteMessageCount :"+remoteMessageCount);
        ...
        Logs.d(Logs.LOG_TAG, "visibleLimit :"+folder.mVisibleLimit);
        ...
        if (remoteMessageCount > 0) {
            ...
            Logs.d(Logs.LOG_TAG, "remoteStart :"+remoteStart+" remoteEnd :"+remoteEnd);
         ...
         */
        Logs.d(Logs.LOG_TAG, "unsyncedMessages.size() :"+unsyncedMessages.size());
...
//        }
        ...
        Logs.d(Logs.LOG_TAG, "fetch title and text :"+(Global.set_download_select == Global.set_download_select_body));
        if(Global.set_download_select == Global.set_download_select_body){
        ...
        try {
            ...
            while (mailboxes.moveToNext()) {
                ...
                Cursor upsyncs2 = resolver.query(EmailContent.Message.UPDATED_CONTENT_URI,
                        EmailContent.Message.ID_PROJECTION,
                        MessageColumns.MAILBOX_KEY + "=?", mailboxKeyArgs,
                        null);
                try {
...
//                    pruneCachedAttachments(accountId);
    ...
    public void sendPendingMessagesSynchronous(final EmailContent.Account account,
        ...
        try {
            ...
            mContext.sendBroadcast(new Intent(Global.ACTION_SENDMESSAGE_BROADCASET)); //�������͹㲥
        } catch (MessagingException me) {
    ...
    public void checkMail(final long accountId, final long tag, final MessagingListener listener) {
    	Logs.d(Logs.LOG_TAG, "syncOneAccount checkMail :");
