...
package com.mail163.email.mail;
...
import com.mail163.email.mail.store.ImapStore;
import com.mail163.email.mail.store.LocalStore;
import com.mail163.email.mail.store.Pop3Store;
...
public abstract class Store {
    ...
    /**
     * java反射机制能够知道类名而不实例化对象的状态下，获得对象的属性或方法调用。
     * 一般都是在配置文件中才用的。hibernate的映射文件,spring的配置文件都是通过反射
     */
    ...
    private static Store instantiateStore(String uri, Context context)
            throws MessagingException {
        Store store = null;
        if (uri.startsWith(STORE_SCHEME_IMAP)) {
            store = ImapStore.newInstance(uri,context,null);
        } else if (uri.startsWith(STORE_SCHEME_POP3)) {
            store = Pop3Store.newInstance(uri,context,null);
        } else if (uri.startsWith(STORE_SCHEME_LOCAL)) {
            store = LocalStore.newInstance(uri,context,null);
        }
    ...
    public synchronized static Store getInstance(String uri, Context context,
        ...
        if (store == null) {
            ...
            /*------实例化实现------*/
            store = instantiateStore(uri, context);
            if (store != null) {
    ...
    public Class<? extends android.app.Activity> getSettingActivityClass() {
        ...
        return com.mail163.email.activity.setup.AccountSetupIncoming.class;
    }
