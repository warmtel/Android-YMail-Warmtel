...
public class Rfc822Output {
    private static final String TAG = "Rfc822Output";
    private static final Pattern PATTERN_START_OF_LINE = Pattern.compile("(?m)^");//(?m)是打开多行模式的开关，^是匹配一行的开头
    private static final Pattern PATTERN_ENDLINE_CRLF = Pattern.compile("\r\n");
    private static boolean isContentType = false; //false:Content-Type: text/plain ; true :text/html 用于回复，转发中
    ...
    /*package*/ static String buildBodyText(Context context, Message message,
        ...
        //================2013.7.3================
        String quotedHtmlText = body.mHtmlReply;

        if (quotedHtmlText != null) {
            // fix CR-LF line endings to LF-only needed by EditText.
            Matcher matcher = PATTERN_ENDLINE_CRLF.matcher(quotedHtmlText);
            quotedHtmlText = matcher.replaceAll("\n");
        }
        isContentType = false;
        ...
        if (isReply) {
            ...
            }else if (quotedHtmlText != null) {
                Matcher matcher = PATTERN_START_OF_LINE.matcher(quotedHtmlText);
                text += matcher.replaceAll("");
    ...
    public static void writeTo(Context context, long messageId, OutputStream out,
        ...
        if(!Global.sendMessage.equals("")){
            if(text.endsWith(Global.sendMessage)){//转发回复时不重复出现
...
//    	String attachmentName = attachment.mFileName;
    ...
    private static void writeTextWithHeaders(Writer writer, OutputStream out, String text)
        ...
        //=============2012.3.4===========
        if(isContentType){//变量，定义Content-Type
